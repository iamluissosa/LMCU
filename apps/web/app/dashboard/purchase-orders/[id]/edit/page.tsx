...
export default function EditPurchaseOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const suppliersResponse = await apiClient.get<{ items: any[]; pagination: any }>('/suppliers');
        ...
        // 3. Cargar Orden
        const order = await apiClient.get<any>(`/purchase-orders/${orderId}`);
        if (order) {
            setFormData({
  ...
  const updateItem = (index: number, field: string, value: any) => {
    ...
    if (field === 'productId') {
      ...
      if (product) {
        newItems[index].unitPrice = 0; // O traer el costo anterior si existiera historial
      }
    }
    setItems(newItems);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 relative">
      <div className="flex items-center gap-4">
        ...
        <div>
            <h1 className="text-2xl font-bold text-gray-800">Editar Orden de Compra</h1>
            <p className="text-sm text-gray-500">Editando orden #{orderId.split('-').pop()}</p>
        </div>
      </div>
