...
export default function QuoteDetailPage() {
  ...
  const handleConvertToOrder = async () => {
    ...
    try {
      ...
      alert(`Error al generar pedido: ${err instanceof Error ? err.message : 'Error desconocido'}`);
    }
  };

  if (loading) return <div className="p-10 text-center text-gray-400">Cargando detalles de cotización...</div>;
  if (!quote) return <div className="p-10 text-center text-red-500 font-bold bg-red-500/5 border border-red-500/20 rounded-xl m-10">Cotización no encontrada.</div>;

  const st = STATUS_CONFIG[quote.status] ?? { label: quote.status, color: 'bg-white/5 text-gray-400' };
    ...
    <div className="max-w-5xl mx-auto space-y-6 print:space-y-0 print:p-0">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 print:hidden">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-white">Cotización N° {quote.quoteNumber}</h1>
          <span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${st.color}`}>
            {st.label}
          </span>
        </div>
      ...
      <div className="bg-[#1A1F2C] rounded-2xl shadow-xl border border-white/10 print:bg-white print:text-black print:shadow-none print:border-none print:m-0 print:p-0 overflow-hidden">
        <div className="p-8 md:p-12 print:p-0">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-8 mb-10 border-b border-white/10 pb-8 print:border-b-2 print:border-gray-800">
            <div className="flex gap-6 items-center">
              {quote.company?.logoUrl ? (
                ...
                <img src={quote.company.logoUrl} alt="Logo" className="w-24 h-24 object-contain brightness-110 filter invert print:invert-0" transition-all />
              ) : (
              ...
              <div>
                <h2 className="text-xl font-bold text-white uppercase tracking-wide print:text-gray-800">{quote.company?.name || 'EMPRESA DEMO'}</h2>
                <div className="text-sm text-gray-400 space-y-0.5 mt-1 print:text-gray-600">
                  <p><span className="font-semibold text-gray-300 print:text-gray-700">RIF:</span> {quote.company?.rif}</p>
            ...
            <div className="text-right">
              <h1 className="text-4xl font-black text-white uppercase tracking-tighter mb-2 print:text-gray-800">COTIZACIÓN</h1>
              <p className="text-xl font-mono font-semibold text-blue-400">{quote.quoteNumber}</p>
              
              <div className="mt-4 text-sm text-gray-400 space-y-1 print:text-gray-600">
                <p><span className="font-semibold text-gray-300 print:text-gray-700">Fecha Emisión:</span> {new Date(quote.issueDate).toLocaleDateString('es-VE')}</p>
                {quote.expiresAt && (
                   <p><span className="font-semibold text-red-400">Válida hasta:</span> {new Date(quote.expiresAt).toLocaleDateString('es-VE')}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mb-8 grid grid-cols-1 md:grid-cols-2 gap-8 print:grid-cols-2">
            <div>
              <h3 className="text-xs font-bold text-gray-600 uppercase tracking-wider mb-2 border-b border-white/10 pb-1 print:text-gray-400 print:border-gray-300">Cotizado a:</h3>
              <p className="font-bold text-white text-lg uppercase print:text-gray-800">{quote.client.name}</p>
              <div className="text-sm text-gray-400 space-y-0.5 mt-2 print:text-gray-600">
                <p><span className="font-medium text-gray-300 print:text-gray-700">Identificación (RIF/CI):</span> {quote.client.rif || 'N/A'}</p>
                {quote.client.address && <p><span className="font-medium text-gray-300 print:text-gray-700">Dirección:</span> {quote.client.address}</p>}
                {quote.client.phone && <p><span className="font-medium text-gray-300 print:text-gray-700">Teléfono:</span> {quote.client.phone}</p>}
                {quote.client.email && <p><span className="font-medium text-gray-300 print:text-gray-700">Correo:</span> {quote.client.email}</p>}
              </div>
            </div>
            ...
            <div className="bg-white/5 rounded-xl p-4 border border-white/10 print:bg-transparent print:border-gray-300 print:rounded-none">
               <h3 className="text-xs font-bold text-gray-600 uppercase tracking-wider mb-2 border-b border-white/10 pb-1 print:text-gray-400 print:border-gray-300">Información Comercial:</h3>
               <div className="text-sm space-y-1.5 mt-2">
                  <div className="flex justify-between">
                    <span className="text-gray-500">Moneda Base:</span>
                    <span className="font-semibold text-white print:text-gray-800">{quote.currencyCode}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Tasa de Cambio (Referencial):</span>
                    <span className="font-mono text-white print:text-gray-800">1 {quote.currencyCode} = {Number(quote.exchangeRate).toLocaleString('es-VE')} VES</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Condición de Pago:</span>
                    <span className="text-gray-300 print:text-gray-800">Contado (Sujeto a aprobación)</span>
                  </div>
               </div>
            </div>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-white/10 text-white print:bg-gray-200 print:text-black print:border-b-2 print:border-black">
                  <th className="py-2.5 px-4 text-left font-bold uppercase tracking-wider text-[10px] w-[10%]">Código</th>
                  <th className="py-2.5 px-4 text-left font-bold uppercase tracking-wider text-[10px] w-[40%]">Descripción</th>
                  <th className="py-2.5 px-4 text-center font-bold uppercase tracking-wider text-[10px] w-[10%]">Cantidad</th>
                  <th className="py-2.5 px-4 text-right font-bold uppercase tracking-wider text-[10px] w-[15%]">Precio Unit.</th>
                  <th className="py-2.5 px-4 text-right font-bold uppercase tracking-wider text-[10px] w-[15%]">Total</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5 print:divide-gray-300">
                {quote.items.map((item, idx) => (
                  <tr key={item.id} className={`${idx % 2 === 0 ? 'bg-white/2' : 'bg-white/0'} hover:bg-white/5 transition-colors print:bg-white`}>
                    <td className="py-3 px-4 font-mono text-[11px] text-gray-500">{item.product?.code || 'SRV'}</td>
                    <td className="py-3 px-4">
                      <p className="font-medium text-white uppercase print:text-gray-800">{item.product?.name || item.serviceCategory?.name}</p>
                      {item.description && <p className="text-xs text-gray-500 mt-0.5">{item.description}</p>}
                    </td>
                    <td className="py-3 px-4 text-center text-gray-300 print:text-black">{Number(item.quantity).toLocaleString('es-VE')}</td>
                    <td className="py-3 px-4 text-right font-mono text-gray-400 print:text-gray-600">{fmt(item.unitPrice, quote.currencyCode)}</td>
                    <td className="py-3 px-4 text-right font-mono font-semibold text-white print:text-gray-800">{fmt(item.totalLine, quote.currencyCode)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="flex flex-col md:flex-row justify-between items-end gap-8 pb-5">
             <div className="w-full md:w-1/2">
                <h3 className="text-xs font-bold text-gray-600 uppercase tracking-wider mb-2 border-b border-white/10 pb-1 print:text-gray-400 print:border-gray-300">Observaciones / Términos:</h3>
             ...
             <div className="w-full md:w-[35%] bg-white/5 p-4 rounded-xl border border-white/10 print:bg-transparent print:border-none print:p-0">
                <div className="space-y-2 text-sm">
                  ...
                  <div className="flex justify-between text-gray-400 print:text-gray-600">
                    <span>IVA</span>
                    <span className="font-mono border-b border-white/10 pb-2 print:border-gray-300">{fmt(quote.taxAmount, quote.currencyCode)}</span>
                  </div>
                  <div className="flex justify-between text-white font-black text-xl pt-1 print:text-black">
                    <span>TOTAL</span>
                    <span className="font-mono text-blue-400 print:text-black">{fmt(quote.totalAmount, quote.currencyCode)}</span>
                  </div>
                </div>
             </div>
          </div>
