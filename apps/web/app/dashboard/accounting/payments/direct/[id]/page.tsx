...
export default function EditDirectPaymentPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [catRes, dptRes, rateData, paymentRes, userRes] = await Promise.all([
          ...
          apiClient.get<any>(`/payments-out/${id}`),
          apiClient.get<any>('/users/me')
        ]);

        if (userRes?.roleName !== 'ADMIN' && !(userRes?.permissions || []).includes('payments.edit')) {
        ...
        if (paymentRes) {
          setPaymentData({
            paymentDate: new Date(paymentRes.paymentDate).toISOString().split('T')[0],
            method: paymentRes.method || 'TRANSFER_VES',
            bankName: paymentRes.bankName || '',
            reference: paymentRes.reference || '',
            exchangeRate: Number(paymentRes.exchangeRate) || 1,
            currencyCode: paymentRes.currencyCode || 'USD',
            notes: paymentRes.notes || ''
          });
          
          if (paymentRes.expenseItems && paymentRes.expenseItems.length > 0) {
            setItems(paymentRes.expenseItems.map((i: any) => ({
        ...
        console.error('Error cargando catálogos o pago', err);
        alert('Error al cargar la información del Gasto Directo');
      }
    };
    fetchData();
  }, [id]);
  ...
  const handleSubmit = async () => {
    ...
    if (!confirm(`¿Confirmar edición de gasto por $${totalAmount.toFixed(2)}?`)) return;
    ...
    try {
      ...
      await apiClient.patch(`/payments-out/${id}`, payload);
      alert('✅ Gasto Simplificado editado exitosamente.');
      router.push('/dashboard/accounting/payments');
    } catch (error: Error | any) {
      console.error(error);
      const msg = error.message || 'Error desconocido';
      alert(`❌ Error al editar gasto: ${msg}`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-black text-white flex items-center gap-2">
            <Receipt className="text-orange-500" /> Editar Gasto Directo
          </h1>
          <p className="text-gray-400 text-[10px] font-bold uppercase tracking-widest mt-1">
            Modificación de Egresos sin factura
          </p>
        </div>
      </div>
          ...
          <div className="bg-[#1A1F2C] text-white p-8 rounded-[2rem] shadow-2xl border border-white/10 relative overflow-hidden">
            ...
            <div className="space-y-4 mb-8">
               ...
               <span className="text-4xl font-black tracking-tighter text-white block">
                 <span className="text-orange-500 text-2xl mr-1">{paymentData.currencyCode === 'USD' ? '$' : paymentData.currencyCode === 'EUR' ? '€' : 'Bs.'}</span>
            ...
            <button onClick={handleSubmit} disabled={loading || totalAmount <= 0}
              className="w-full bg-orange-600 hover:bg-orange-500 disabled:opacity-50 text-white font-black py-5 rounded-2xl flex items-center justify-center gap-3 shadow-2xl shadow-orange-500/20 transition-all uppercase tracking-widest text-xs active:scale-95 disabled:hover:bg-orange-600 disabled:cursor-not-allowed">
              {loading ? 'Procesando...' : <><Save size={20} /> Guardar Cambios</>}
            </button>
            <p className="text-center text-[9px] font-bold text-gray-400/50 mt-4 uppercase tracking-widest">
              Las fechas y descripciones se re-escribirán completamente
            </p>
          </div>
        </div>
