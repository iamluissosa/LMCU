...
export class CompaniesController {
  ...
  async extractRifData(@UploadedFile() file: any) {
    ...
    try {
      ...
      if (file.mimetype === 'application/pdf') {
        ...
        try {
            ...
            console.log('PDF procesado. Longitud texto:', text?.length);
             ...
             throw new Error(`Error lectura PDF: ${pdfError.message || 'Archivo corrupto o encriptado'}`);
        }
      } 
      // B) Procesar Imagen (OCR)
        ...
        try {
            ...
            try {
                ...
                console.warn('Fallo carga Español, usando Inglés (eng)...', langError);
            ...
            throw new Error(`Fallo OCR: ${ocrError && typeof ocrError === 'object' && 'message' in ocrError ? ocrError.message : ocrError}`);
        }
      } 
      else {
        throw new HttpException('Formato no soportado. Usa PDF o Imagen (JPG, PNG)', HttpStatus.BAD_REQUEST);
      }

      console.log('Texto extraído (Inicio):', text.substring(0, 500).replace(/\n/g, ' '));
      ...
      const rifLineIndex = lines.findIndex(l => l.match(/([VJEGPvjegp])[- ]?(\d{8,9})[- ]?(\d)/));
      
      if (rifLineIndex !== -1) {
         // Buscar nombre en la línea del RIF o siguientes
         let rawName = lines[rifLineIndex].replace(/([VJEGPvjegp])[- ]?(\d{8,9})[- ]?(\d)/, '').trim();
         ...
         if (rawName.length < 3 && lines[rifLineIndex + 1]) {
             ...
             if (lines[rifLineIndex + 2] && !lines[rifLineIndex + 2].includes('DOMICILIO')) {
      ...
      const addressStartIndex = lines.findIndex(l => l.toUpperCase().includes('DOMICILIO FISCAL'));
      if (addressStartIndex !== -1) {
        ...
        for (let i = addressStartIndex; i < Math.min(addressStartIndex + 6, lines.length); i++) {
            let line = lines[i].replace(/DOMICILIO FISCAL/i, '').trim();
            if(!line) continue;

            // Detectar Ciudad/Estado si están explícitos o inferirlos
            // Lista simple de estados para detección
            const states = ['CARABOBO', 'DISTRITO CAPITAL', 'MIRANDA', 'ARAGUA', 'LARA', 'ZULIA', 'BOLIVAR', 'FALCON', 'MERIDA', 'TACHIRA', 'TRUJILLO', 'YARACUY', 'PORTUGUESA', 'SUCRE', 'NUEVA ESPARTA', 'ANZOATEGUI', 'MONAGAS', 'DELTA AMACURO', 'AMAZONAS', 'APURE', 'BARINAS', 'COJEDES', 'GUARICO', 'VARGAS', 'LA GUAIRA'];
            ...
            for (const s of states) {
                if (line.toUpperCase().includes(s)) {
                    ...
                    const potentialCity = line.toUpperCase().replace(s, '').replace('ESTADO', '').replace('EDO.', '').replace(',', '').trim();
        ...
        if (city) {
             ...
             extractedData.city = city.charAt(0).toUpperCase() + city.slice(1).toLowerCase(); 
        }
      }
      ...
      return extractedData;
       ...
       throw new HttpException(error.message || 'Error procesando el documento', HttpStatus.INTERNAL_SERVER_ERROR);
    }
  }
